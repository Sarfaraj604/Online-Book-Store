...
function BookSection() {
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      <h1 className="text-3xl sm:text-4xl md:text-5xl text-center mb-10 font-bold text-[#000000]">
        Our Best Picks
      </h1>
      ...
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={index}>
            <Link to="/book">
              <div className="bg-white shadow-lg rounded-lg overflow-hidden text-center">
                <img src={slide.image} alt={slide.title} className="w-full h-[16rem] sm:h-[20rem] object-cover" />
                <div className="p-2">
                  <h3 className="text-lg sm:text-xl font-bold text-black">{slide.title}</h3>
                  <p className="text-black text-base sm:text-lg font-normal">{slide.description}</p>
                </div>
              </div>
            </Link>
          </SwiperSlide>
